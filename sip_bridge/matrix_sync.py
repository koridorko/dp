...
def parse_sync_for_answer_with_party_id(sync_response, room_id: str, call_id: str) -> tuple[str, str] | None:
    """Return (sdp, party_id) from m.call.answer in sync_response for room_id/call_id."""
    if not getattr(sync_response, "rooms", None) or not getattr(sync_response.rooms, "join", None):
        return None
...
def parse_sync_for_candidates_from_response(
    ...
    return [t[:3] for t in _parse_candidates_with_party(sync_response, room_id, call_id)]
...
def parse_sync_for_candidates_by_party(
    ...
    out = []
    for cand, _mid, _idx, pid in _parse_candidates_with_party(sync_response, room_id, call_id):
...
def parse_sync_for_hangup_from_response(sync_response, room_id: str, call_id: str) -> tuple[bool, str, str]:
    ...
    for event in events_from_room(room):
        ...
        if ev_type == "m.call.hangup" and content.get("call_id") == call_id:
            return (True, content.get("reason", "") or "", content.get("party_id", "") or "")
    return (False, "", "")


def log_sync_call_events_from_response(sync_response, room_id: str, call_id: str) -> None:
    ...
    for event in events_from_room(room):
        ev_type, content = get_event_type_and_content(event)
        if not (ev_type and ev_type.startswith("m.call.")) or content.get("call_id") != call_id:
            continue
        pid = content.get("party_id", "")
        if ev_type == "m.call.answer":
            answer_sdp = (content.get("answer") or {}).get("sdp") or ""
            print(f"[Element->bot] m.call.answer party_id={pid!r} answer_sdp_len={len(answer_sdp)}")
            ...
            print(f"[Element->bot] m.call.hangup party_id={pid!r} reason={content.get('reason', '')!r}")
        else:
    ...
    for attr in ("timeline", "state"):
        ...
        for event in container.events:
            ...
            if ev_type != "m.room.member":
                continue
            state_key = getattr(event, "state_key", None) or (getattr(event, "source", None) or {}).get("state_key")
...
def find_room_with_only_user_from_sync_response(sync_response, bot_user_id: str, target_user_id: str) -> str | None:
...
def find_room_with_only_user_via_joined_rooms_api(client, bot_user_id: str, target_user_id: str) -> str | None:
    want = {bot_user_id, target_user_id}
        ...
        for e in events:
            ...
            if typ and typ.startswith("m.call."):
                out["call_events_seen"].append({
                    ...
                    "call_id_match": content.get("call_id") == call_id if isinstance(content, dict) else False,
                })
    return out


def dump_sync_debug_from_response(sync_response, room_id: str, call_id: str, dump_path: str = "sync_debug.json") -> None:
...
def patch_nio_call_candidates_schema() -> None:
    ...
    try:
        ...
        schema = getattr(schemas.Schemas, "call_candidates", None)
        if isinstance(schema, dict):
            items = (
                (schema.get("properties") or {}).get("content", {}).get("properties", {}).get("candidates", {}).get("items")
            )
